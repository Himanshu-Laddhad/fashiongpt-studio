...
async def scrape_zara(query: str, output_dir: Path) -> pd.DataFrame:
    ...
    try:
        ...
        # Build search URL (no gender section restriction so all products are returned)
        search_url = f"https://www.zara.com/us/en/search?searchTerm={query.replace(' ', '%20')}"
            ...
            if result.success and result.html:
                ...
                for idx, product in enumerate(product_divs[:15]):  # Try 15, keep best 10
                    try:
                        ...
                        # Extract image and upgrade to highest available resolution
                        ...
                        if img_elem:
                            ...
                            # Zara CDN: upgrade thumbnail sizes to full resolution
                            image_url = (
                                image_url.replace('/w/235/', '/w/1024/')
                                         .replace('/w/100/', '/w/1024/')
                                         .replace('/w/200/', '/w/1024/')
                                         .replace('/w/300/', '/w/1024/')
                                         .replace('-thumb', '')
                            )
...
def generate_fallback_zara(query: str, count: int = 10) -> pd.DataFrame:
    ...
    unsplash_ids = [
        "1490481651871-ab68de25d43d", "1539533018447-63fcce2678e3",
        "1445205170230-053b83016050", "1512436991641-6745cdb1723f",
        "1469334031218-e382a71b716b", "1515886657613-9f3515b0c78f",
        "1509631179647-0177331693ae", "1558769132-cb1aea458c5e",
        "1542295669297-4d352b042bca", "1551488831-00ddcb6c6bd3",
    ]
    for i in range(count):
        ...
        products.append({
            ...
            'brand': 'Zara',
            'url': f"https://www.zara.com/us/en/search?searchTerm={query.replace(' ', '%20')}",
        })
