"""
AI-Powered Fashion Trend Analysis - Fully Agent-Based
Using FREE Groq API (Llama 3.3 70B) for all insights, recommendations, and summaries
"""
...
- Return ONLY JSON"""
    ...
    # Execute in thread pool (use get_running_loop — get_event_loop is deprecated)
...
async def generate_executive_summary_ai(
    query: str,
    trend_analysis: Dict,
    brand_customizations: Dict,
) -> str:
    """
    Ask Groq to write a concise executive summary that synthesises the full
    trend + brand analysis.  Falls back to a simple template if the API call
    fails.
    """

    if not client:
        return _fallback_summary(query, trend_analysis)

    colors   = ", ".join(trend_analysis.get("dominant_palette", [])[:4])
    materials = ", ".join(trend_analysis.get("materials", [])[:3])
    vibes    = ", ".join(trend_analysis.get("aesthetic_vibes", [])[:3])
    trends   = ", ".join(trend_analysis.get("key_trends", [])[:4])
    confidence = trend_analysis.get("market_confidence", "high")

    on_summary  = brand_customizations.get("old_navy", {}).get("summary", "")[:120]
    br_summary  = brand_customizations.get("banana_republic", {}).get("summary", "")[:120]
    gap_summary = brand_customizations.get("gap", {}).get("summary", "")[:120]

    prompt = f"""You are a senior fashion market analyst writing a concise executive brief.
...
BRAND INTERPRETATIONS (brief):
- Old Navy: {on_summary}
- Banana Republic: {br_summary}
- GAP: {gap_summary}

Write a 3-4 sentence executive summary that:
1. States the headline trend insight for "{query}"
2. Highlights the standout colors/materials
3. Notes the market confidence and overall aesthetic direction
4. Closes with a one-line strategic recommendation
...
Return ONLY the summary text — no JSON, no bullet points, no headers."""
    ...
    def _call():
        r = client.chat.completions.create(
            model=GROQ_MODEL,
            messages=[
                {"role": "system", "content": "You are a concise, authoritative fashion market analyst."},
    ...
    try:
        ...
        print(f"   Warning: executive summary AI call failed ({e}), using template")
        return _fallback_summary(query, trend_analysis)


def _fallback_summary(query: str, trend_analysis: Dict) -> str:
    trends   = ", ".join(trend_analysis.get("key_trends", [])[:3]) or query
    colors   = ", ".join(trend_analysis.get("dominant_palette", [])[:3]) or "neutral tones"
    vibes    = ", ".join(trend_analysis.get("aesthetic_vibes", [])[:2]) or "contemporary"
    materials = ", ".join(trend_analysis.get("materials", [])[:2]) or "premium fabrics"
    return (
        f"Market analysis reveals '{query}' trending with {trends}. "
        f"Dominant palette features {colors} with {materials}. "
        f"Overall aesthetic direction: {vibes}. "
        f"Strategic opportunity exists across all three brand tiers."
    )
